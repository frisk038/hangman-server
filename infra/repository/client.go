...
const updateUserName = "UPDATE userscore SET name = COALESCE('FSK', name) WHERE userid = 'ccff41b1-4e90-45ff-9567-383456dffaba' AND secretnum = 5 AND name IS NULL RETURNING userid;"
const selectTopPlayer = "select name, score from userscore where SECRETNUM = $1 AND NAME is not null  ORDER BY score LIMIT 5;"
